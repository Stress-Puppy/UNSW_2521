...
EdgeValues edgeBetweennessCentrality(Graph g) {
	...
	for (i = 0; i < V; i++) {
        for (j = 0; j < V; j++) {
			// if (e.values[i][j] != -1.0 && sps.next[i][j] != -1) {
            ...
            if (sps.next[i][j] != -1) {
				...
				e.values[i][j] += 1;
				i = record;
				// // if the shortest path is from the i tp j directly
				// if (sps.next[i][j] == j) {
				// 	e.values[i][j] += 1;
				// }
				// // if the shortest path is not from the i tp j directly
				// else {
				// 	e.values[i][sps.next[i][j]] += 1;
				// 	e.values[sps.next[i][j]][j] += 1;
